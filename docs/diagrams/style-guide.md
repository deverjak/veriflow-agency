...
name: VeriFlow Agency
slug: veriflow-agency
source-url: none
created: 2026-09-09
updated: 2026-09-09
notes: Tokens taken from docs/site/index.html
-->
...
|---|---|---|---|
| `paper` | Page background, default node fill | `#fbfaf7` (bone) | `#14140f` (bistre) |
| `paper-2` | Diagram container bg, secondary fill | `#ffffff` | `#1e1e17` |
| `ink` | Primary text, primary stroke | `#1b1a17` (bistre) | `#f0eee7` (bone) |
| `muted` | Secondary text, default arrow stroke | `#504d45` | `#bab7ac` |
| `soft` | Sublabels, boundary labels | `#6b675e` | `#9d9a90` |
| `rule` | Hairline borders | `rgba(27,26,23,0.16)` | `rgba(240,238,231,0.18)` |
| `rule-solid` | Stronger borders, baselines | `#cbc7be` | `#403e34` |
| `accent` | Focal / 1–2 max per diagram | `#1a6743` (agency green) | `#7fd0a3` |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(26,103,67,0.10)` | `rgba(127,208,163,0.14)` |
| `link` | HTTP/API calls, external arrows | `#2f5d8c` | `#7ba3d6` |

> **Brand palette source:** `paper` and `ink` are the tokens `docs/site/index.html` ships (`--bg #fbfaf7`, `--fg #1b1a17`, and their dark counterparts). The rest is that same family pushed for contrast, because diagram type runs at 9–16px and the site's `--dim` reads at about 2.9:1 on bone: `muted` and `soft` are darkened until both clear WCAG AA on `paper`, `accent` is the site's green one step deeper, and `link` is a slate blue for a role the site does not name. Nothing here introduces a second hue.
...
Any `rgba(27,26,23, X)` in light becomes `rgba(236,234,227, X)` in dark. Same opacities, RGB flipped. The accent lifts from the deep green to a mint that reads on bistre paper.
