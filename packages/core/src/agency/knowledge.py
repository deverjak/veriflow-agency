...
def _view(run, rec: dict, finding: dict, decision: dict | None,
    ...
    view = {
        ...
        # against a graph impact set, which a line number cannot do — the
        # function moves, the name does not.
        "symbol": ((a.get("symbol") or {}).get("name")
                   if isinstance(a.get("symbol"), dict) else None),
...
def _impact_scope(ev: Path) -> tuple[set[str], set[str]]:
    """Files and symbols the graph says this run's change reaches.

    Read off `evidence/impact.json`, which the same preparation has already
    paid for — this asks the graph nothing new. The shapes are the code-graph
    tool's, not ours, so both the string and object forms are accepted: this
    is a boundary, and a list of blast-radius files is not worth crashing a
    run over.
    """
    raw = read_json(ev / "impact.json", default=None)
    if not isinstance(raw, dict):
        return set(), set()
    files: set[str] = set()
    for item in raw.get("impacted_files") or []:
        if isinstance(item, str):
            files.add(item)
        elif isinstance(item, dict):
            name = item.get("file_path") or item.get("file") or item.get("path")
            if name:
                files.add(str(name))
    symbols: set[str] = set()
    for node in raw.get("impacted_nodes") or []:
...
def here(known: list[dict], ev: Path, files: list[str]) -> list[dict]:
    ...
    newest. That is a question about SHAPE, not about ranking — which is why
