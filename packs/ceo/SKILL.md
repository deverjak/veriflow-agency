...
**Score** 0–100 — your own confidence, and nothing is dropped for being low. It is read as calibration: a pack that scores everything 90 and has half its bets turned down is a pack whose scores mean nothing. What a strategy claim has to clear instead is its evidence: a bet stands on a page this run opened, not on a number. Score decides one thing only — if you write more than `review.limits` allows, the best-scored survive.
