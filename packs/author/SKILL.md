...
6. **Where a finding sits** — a `code` evidence item, whose `locator` carries `file` + `line` relative to the project root, `commit` = **all 40 characters** of `target.headRefOid`, `endLine`, `snippet` of the whole block, `symbol` where the graph can give one. It is one kind of proof among several, not a field every output has: a type whose subject is not the code says so with `outputs.<type>.anchor: none` and then owes `evidence.required` instead. A finding that points nowhere does not survive `agency ingest`, so this section is not decoration.
