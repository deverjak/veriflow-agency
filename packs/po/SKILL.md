...
- **`promote` and `decide` ask before they run, unless the run is unsupervised.** Both are pre-authorized only where nobody could answer a prompt anyway: inside a chain (`agency chain …`), or when the run was started with `--unattended` (the extension calls that **Unsupervised**). In an ordinary `agency run po`, Claude Code will ask a person to approve each call — that pause is deliberate: promoting a draft notifies people, and a disposition move is meant to be signed by a person as much as by you. Do not work around it by calling `gh` yourself. `snapshot`, `comment` and `draft --title` stay pre-authorized either way — they post nothing that notifies anyone.
